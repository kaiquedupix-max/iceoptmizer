...
const files=['.gitignore','.gitattributes','README.md','THIRD-PARTY-NOTICES.md','build.ps1','catalog.json',...walk('src'),...walk('scripts'),...walk('tools'),...walk('tests'),...walk('assets')];
const isText=p=>/\.(?:md|ps1|json|mjs|cs|manifest|bat|ini|txt|config|svg)$|^\./.test(p)&&fs.readFileSync(p).equals(Buffer.from(fs.readFileSync(p,'utf8'),'utf8'));
